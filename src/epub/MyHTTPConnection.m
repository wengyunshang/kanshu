...
- (NSObject<HTTPResponse> *)httpResponseForMethod:(NSString *)method URI:(NSString *)path
{
	...
	if ([method isEqualToString:@"POST"]){
        ...
        if([path rangeOfString:@"/delFile"].location == 0){
            ...
            //删除数据库信息
            [[PenSoundDao sharedPenSoundDao]delBook4Id:[[self parseGetParams] objectForKey:@"bookId"]];//删除书本数据库信息
                                                [[WxxPopView sharedInstance] showPopText:@"你已成功删除一本书"];
//            [[WxxAlertView sharedInstance]showWithTextWithTime:@"你已成功删除一本书"];
...
-(NSString *)generateLists:(NSString *)realPath{
    /*******读取数据库 */
    NSMutableArray* booksArr = [[PenSoundDao sharedPenSoundDao]selectMyBookList:yesDown];
    NSMutableString* filesStr = [[NSMutableString alloc] init];
    for(BookData *data in booksArr) {
        //id小于0的书籍是用户自己上传的，网页只能显示玩家自己上传的书籍
        if ([data.bbook_id intValue] < 0) {
            NSString *path = [self filePathForURI:data.bbook_file allowDirectory:YES];
            BOOL isDir = TRUE;
            BOOL isExist = [[NSFileManager defaultManager] fileExistsAtPath:path isDirectory:&isDir];
            if(isExist && isDir){
                [filesStr appendFormat:@"<tr><td width='20' class='dir'></td><td><a href=\"%@/\"> %@ </a></td><td width='50'><a class=\"btn btn-update\" href=\'javascript:delFile(\"%@\")\'>%@</a></td></tr>",data.bbook_file, data.bbook_file,data.bbook_file,NSLocalizedString(@"delete", nil)];
                
                //             [filesStr appendFormat:@"<tr><td width='20' class='dir'></td><td><a href=\"%@/\"> %@ </a></tr>",file, file];
            }else{
                //[filesStr appendFormat:@"<tr><td width='20' class='dir'></td><td><a href=\"%@/\"> %@ </a></tr>",file, file];
                [filesStr appendFormat:@"<tr><td width='20' class='file'></td><td><a href=\"%@\"> %@ </a></td><td width='50'><a class=\"btn btn-update\" href=\'javascript:delFile(\"%@\",\"%@\")\'>%@</a></td></tr>",data.bbook_file, data.bbook_name,data.bbook_file,data.bbook_id,NSLocalizedString(@"delete", nil)];
            }
        }
    }
...
- (void) processStartOfPartWithHeader:(MultipartMessageHeader*) header {
    ...
    //创建书籍
    [WxxBookSelect createNewBook:filename];
