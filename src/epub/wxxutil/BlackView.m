...
+(void)orgXAnimationRepeat:(UIView *)v selfOrgx:(float)selfOrgx  orgx:(float)orgx duration:(float)dur{
    ...
    [UIView animateWithDuration:dur animations:^{
        ...
        if (CGRectGetMinX(v.frame) >= 320) {
                    detailRect.origin.x = 0;
        }

        [v setFrame:detailRect];
    }completion:^(BOOL finished){
//        NSLog(@"%f",selfOrgx == CGRectGetMinX(v.frame)? orgx:selfOrgx);
        [self orgXAnimationRepeat:v selfOrgx:selfOrgx orgx:orgx duration:dur];
...
+(void)sharke :(UIView *)vv num:(float)num{
    ...
    if (vv.frame.origin.x<=num) {
        vv.frame = CGRectMake(vv.frame.origin.x+200, vv.frame.origin.y, vv.frame.size.width, vv.frame.size.height);
    }else {
        vv.frame = CGRectMake(vv.frame.origin.x-200, vv.frame.origin.y, vv.frame.size.width, vv.frame.size.height);
    }
//    if (vv.frame.origin.x==num) {
...
+(void)sharke :(UIView *)vv num:(float)num movewidth:(float)movewidth dur:(float)dur{
    ...
    //  [UIView setAnimationRepeatAutoreverses:NO];
    if (vv.frame.origin.x<= num) {
        vv.frame = CGRectMake(vv.frame.origin.x+movewidth, vv.frame.origin.y, vv.frame.size.width, vv.frame.size.height);
    }else {
//        NSLog(@"%f",vv.frame.origin.x);
        vv.frame = CGRectMake(vv.frame.origin.x-movewidth, vv.frame.origin.y, vv.frame.size.width, vv.frame.size.height);
    }
