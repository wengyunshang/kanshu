...
- (void)downloadWithURL:(NSURL *)url url2:(NSURL *)url2arg delegate:(id<SDWebDataManagerDelegate>)delegate refreshCache:(BOOL)refreshCache
{ 
...
{
	if (!url || (!retryFailed && [failedURLs containsObject:url]))
    {
        return;
    }
...
- (void)dataCache:(SDDataCache *)dataCache didFindData:(NSData *)aData forKey:(NSString *)key userInfo:(NSDictionary *)info
{
	...
	{
		[delegate webDataManager:self didFinishWithData:aData isCache:YES url2:self.url2];
	}
}
    ...
    for (NSInteger idx = [downloaders count] - 1; idx >= 0; idx--)
    {
        ...
        if (aDownloader == downloader)
        {
            ...
            if (aData)
            {
				if ([delegate respondsToSelector:@selector(webDataManager:didFinishWithData:isCache:)]) 
				{
					[delegate webDataManager:self didFinishWithData:aData isCache:NO url2:self.url2];
				}
            }
            else
            {
